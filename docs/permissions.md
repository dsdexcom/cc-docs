...
> **Note:** These are system-wide paths that require administrator privileges. Claude Code checks MDM/OS-level policies first, then falls back to managed settings files. Only one source is used.
