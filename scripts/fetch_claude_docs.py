...
def save_manifest(docs_dir: Path, manifest: dict) -> None:
    ...
    manifest["description"] = (
        "Claude Code documentation manifest. Keys are filenames, append to base_url for"
        " full URL."
    )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logger.warning(
                        "XMLParser security parameters not available, using default"
                        " parser"
                    )
...
def discover_claude_code_pages(
    ...
    try:
        ...
        try:
            ...
            # Fallback for older Python versions
            logger.warning(
                "XMLParser security parameters not available, using default parser"
            )
            root = ET.fromstring(response.content)
        ...
        logger.info(
            f"Discovered {len(claude_code_pages)} Claude Code documentation pages"
        )
...
def validate_markdown_content(content: str, filename: str) -> None:
    ...
    if indicator_count < 3:
        raise ValueError(
            f"Content doesn't appear to be markdown (only {indicator_count} markdown"
            " indicators found)"
        )
...
def fetch_markdown_content(
    path: str, session: requests.Session, base_url: str
) -> Tuple[str, str]:
    """
    ...
    for attempt in range(MAX_RETRIES):
        try:
            response = session.get(
                markdown_url, headers=HEADERS, timeout=30, allow_redirects=True
            )
...
def fetch_changelog(session: requests.Session) -> Tuple[str, str]:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            response = session.get(
                changelog_url, headers=HEADERS, timeout=30, allow_redirects=True
            )
    ...
    with requests.Session() as session:
        ...
        for i, page_path in enumerate(documentation_pages, 1):
            ...
            try:
                ...
                content = clean_markdown_content(content=content, filename=filename)
